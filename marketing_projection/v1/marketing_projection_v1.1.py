...
def calculate_weighted_group_averages(df, group_column):
    def weighted_avg_all_columns(group):
        # Get the number of elements in the group
        n = len(group)

        # Create weights that increase exponentially with index position
        weights = np.exp(np.linspace(0, 1, n))
        weights = weights / weights.sum()

        # Get numeric columns except the group column
        numeric_cols = group.select_dtypes(include=[np.number]).columns
        numeric_cols = numeric_cols[numeric_cols != group_column]

        # Calculate weighted average for each numeric column
        results = {}
        for col in numeric_cols:
            weighted_avg = (group[col] * weights).sum()
            results[col] = weighted_avg

        return pd.Series(results)

    # Apply the weighted average calculation to each group
    results = df.groupby(group_column).apply(weighted_avg_all_columns)
...
# Using merge if indices don't match
...
# First merge with matching groups
merged_df = df_final_value[['su_week', 'group', 'total_costs', 'platform', 'traffic_type']].merge(
    df_combined_mean_adjusted_enc_min_pct,
    on='group',
    how='left'
)

# For groups that don't match, fill with values from the 'overall' group
overall_values = df_combined_mean_adjusted_enc_min_pct[
    df_combined_mean_adjusted_enc_min_pct['group'] == 'mean_overall'
    ].iloc[0]  # Get the first row since we only need the values once per group

# Fill NaN values with the overall values
final_df_cal = merged_df.fillna(overall_values)
