...
def predict_vip_status_v2(df_model_full, window_models, decision_threshold=0.96):
...
# [IN] Setup Directory
base_dir = '/tmp/models'
model_dir = os.path.join(base_dir, 'Potential_VIP')
...
try:
    dataset_query = """
    select * from guyb_sandbox.models_datasets_potential_vip_v2
    """
...
files_to_download = {
    "existing_users.sql": "models/Potential_VIP/existing_users.sql",
    "model_features_v2.txt": "models/Potential_VIP/model_features_v2.txt",
    "pot_vip_v2.pkl": "models/Potential_VIP/pot_vip_v2.pkl",
    "enp.sql" : "models/Potential_VIP/enp.sql"
}
...
try:
    existing_users_path = os.path.join(model_dir, 'existing_users.sql')
...
try:
    enp_users_path = os.path.join(model_dir, 'enp.sql')
...
# [IN] Load Models
try:
    model_path = os.path.join(model_dir, 'pot_vip_v2.pkl')
...
try:
    with open(os.path.join(model_dir, 'model_features_v2.txt'), 'r') as file:
...
# Create final dataframe with explicit copy
...
predictions_df_final = predictions_df_final.assign(
    ...
    version='v2',
    shadow_mode=False,
    updated_at=datetime.datetime.now().replace(second=0, microsecond=0)
)

# Upload new records
destination_table = 'ml_models.potential_vip'
...
# Get date from 5 days ago
five_days_ago_date = (datetime.datetime.now() - datetime.timedelta(days=5)).date()

# Query to get users from original table who were updated 5 days ago
five_days_ago_query = f""" 
SELECT user_id, case when vip_mentioned = 0 then 'A' else 'B' end as test_group 
FROM `{project_id}.ml_models.potential_vip`
WHERE DATE(updated_at) = '{five_days_ago_date}'
and vip_mentioned = 1
"""
...
# Execute the query to get users from 5 days ago
df_users = client.query(five_days_ago_query).to_dataframe()

# Get current time for the created_at field
current_time = datetime.datetime.now()

# Create the experiment data for experiment_id 1090
df_experiment = pd.DataFrame({
    'experiment_id': [1090] * len(df_users),
    'user_id': df_users['user_id'].values,
    'experiment_group': ['B'] * len(df_users),
    'created_at': [current_time] * len(df_users)
})

# First, delete all existing experiment_id 1090 records directly in BigQuery
delete_query = f"""
DELETE FROM `{project_id}.experiments.fact_experiment`
WHERE experiment_id = 1090
"""
client.query(delete_query).result()  # .result() waits for the query to complete

# Write just the new experiment data to BigQuery in append mode
df_experiment.to_gbq(
    destination_table='experiments.fact_experiment',
    project_id=project_id,
    if_exists='append',
    table_schema=[
        {'name': 'experiment_id', 'type': 'INTEGER'},
        {'name': 'user_id', 'type': 'INTEGER'},
        {'name': 'experiment_group', 'type': 'STRING'},
